...
class TrackMaintenanceMHT:
	...
	def __init__(self, threshold_old_track, threshold_miss_measurement, threshold_new_track, prob_detection, obs_dim, lambda_fa, R, P, kFilter_model, pruning_n, scoring_method, born_p, num_obj_start):
		...
		Args:
			...
			born_p (float): probability of object birth at each time step
			num_obj_start (int): number of objects initialized in simulation
		"""
	...
	def closest_track(self, measurement_index, tracks):
		"""
		Args:
			measurement_index (int): index of measurement to assign as false alarm or part of track
			tracks (list): list of tracks
		Returns:
			min_p (float): lowest probability that measurement is not associated with an object
		"""
		min_p = None
		for track in tracks:
