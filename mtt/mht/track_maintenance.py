...
class TrackMaintenanceMHT:
        ...
        for j, track in enumerate(tracks):
            ...
            for possible_observation in track.possible_observations:
                ...
                if score >= self.threshold_old_track:
                    ...
                    # print("ID, OBS: ", track.obj_id, possible_observation, "OBS: ", track.observations, "SCORE: ",
                    #       track.score)
    ...
    def new_object(self, ts, tracks, measurements, new_tracks):
        ...
        for i, measurement in enumerate(measurements):
            # Check if measurement is in the gate of another object
            # This reduces the errors where two trajectories track the same object consecutively
            # If it is actually a new object, then including it in the trajectory won't harm the predictions that much
            measurement_used = False
            for track in tracks:
                if i in track.possible_observations:
                    measurement_used = True
                    break
            # Measurement is not inside the gate of another object, so create a new track
            if not measurement_used:
                ...
                self.num_objects += 1  # This is the object ID for the next new track created from a measurement
